package intrep.util.magpiebridge.converter;
...
public class ClassConverter {
  ...
  public void convertClass(JavaClass fromClass) {
    ...
    if (!Scene.v().containsClass(className)) {
      ...
      toClass = Scene.v().getSootClass(className);
    ...
    for (IMethod walaMethod : fromClass.getDeclaredMethods()) {
      ...
      toClass.getOrAddMethod(sootMethod);
  ...
  private SootMethod convertMethod(
    ...
    SootMethod toMethod = new SootMethod(
    ...
    if (cfg != null) {
      ...
      if (insts.length > 0) {
        ...
        if (!Modifier.isStatic(modifiers)) {
          ...
          Stmt stmt = Jimple.v().newIdentityStmt(thisLocal, Jimple.v().newThisRef(declaringClassType));
        ...
        for (; startPara < walaMethod.getNumberOfParameters(); startPara++) {
          ...
          Stmt stmt = Jimple.v().newIdentityStmt(paraLocal, Jimple.v().newParameterRef(type, index));
          Position pos = debugInfo.getInstructionPosition(0);
          stmt.addTag(new StmtPositionInfoTag(new StmtPositionInfo(pos, null)));
          stmts.add(stmt);
        }
        ...
        InstructionConverter instConverter = new InstructionConverter(this, walaMethod, localGenerator);
  ...
  /**
   * It adds RefType with the given typeName to Scene and makes sure every RefType
   * is resolved,
   * otherwise
   * {@link soot.jimple.spark.internal.TypeManager#isUnresolved(soot.Type)} will
   * case
  ...
  private RefType addRefTypeToScene(String typeName) {
    ...
    if (!t.hasSootClass())
      Scene.v().forceResolve(typeName, soot.SootClass.SIGNATURES);
    if (!Scene.v().containsType(typeName))
      Scene.v().getOrAddRefType(typeName);
    return t;
  }
  ...
  /**
   * Convert className in wala-format to soot-format, e.g., wala-format:
   * Ljava/lang/String
